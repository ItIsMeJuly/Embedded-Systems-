...
extern "C" void EXTI0_IRQHandler(void)  
{
    ...
    if((GPIOB->IDR & GPIO_IDR_0) == GPIO_IDR_0)
    {
      ...
      GPIOA->ODR |= GPIO_ODR_1;  // turn on PA1
      GPIOA->ODR &= ~GPIO_ODR_4; // turn off PA4
    }
    else
    {
      snprintf(msgBuf, MSGBUFSIZE, "%s", "Counter-Clockwise\r\n");
      GPIOA->ODR |= GPIO_ODR_4;  // turn on PA4
      GPIOA->ODR &= ~GPIO_ODR_1;  //turn off PA1
    }
...
void SetupLedPins()
{
  GPIOA->MODER |= (GPIOA->MODER & ~GPIO_MODER_MODER1) | (0b00 << GPIO_MODER_MODER1_Pos);  // set pin PA* to output.            
  GPIOA->OTYPER &= ~GPIO_OTYPER_OT_1;                                                    // set pin PA* to output type to push-pull

  GPIOA->MODER |= (GPIOA->MODER & ~GPIO_MODER_MODER4) | (0b00 << GPIO_MODER_MODER4_Pos);  // set pin PA* to output.            
